...
Due date: 23:59:59, 18th of December, 2015.
...
1. Each of the news will be given on a new line.
2. The total number of news will never be more than 100 000.
3. The total number of soldiers will never be more than 100 000.
4. The total number of units will never be more than 10 000.
1. Unit names will never contain whitespace (interval, tab, etc.)
1. When listing the soldiers in a given unit, they'll be separated by a comma
and a single whitespace. For example - `[1, 2, 3, 4]`.
1. Your output should contain one line for every `show` command. No trailing
...
5. Units can only be attached to **at most one** other unit. Attempts
to attach a unit a second time, should detach it from the previous one.
6. Soldiers, however, can be in as many units as possible. The following example
...
show soldier 1
```
7. When a soldier dies, he should be removed from all units he's in.
8. Soldier identifiers will be unique.
9. When a range of soldiers die, they'll be always specified
in the correct unit and order.
1. When units are attached with a position, they'll never be attached in
...
```
10. **Important: ** Soldiers which are used as range limits,
we'll be probably used more than once. In other words, if soldier `<soldier_id>`
